...
#define	COMMAND_WRITE_ENABLE	0x06 /* Write Enable command AICI*/
#define COMMAND_SECTOR_ERASE	0xD8 /* Sector Erase command */
#define COMMAND_BULK_ERASE		0xC7 /* Bulk Erase command AICI*/
#define COMMAND_STATUSREG_READ	0x05 /* Status read command AICI*/
...
int SpiFlashWrite(XSpi *SpiPtr, u32 Addr, char Data[],u32 ByteCount, u8 WriteCmd);
...
int main(void)
{
	 ...
	 */
	char Data[] = "Test Flash"; // The data sent to the flash memory
	int Length = strlen(Data);
	Address += PAGE_SIZE;
	Status = SpiFlashWrite(&Spi, Address, Data, Length, COMMAND_QUAD_WRITE);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	...
	for(Index = 0; Index < Length; Index++) {
		xil_printf("%c",ReadBuffer[Index + READ_WRITE_EXTRA_BYTES + QUAD_IO_READ_DUMMY_BYTES]);
	}

	xil_printf("\r\nSuccessfully ran Spi winbond flash quad Example\r\n");
	return XST_SUCCESS;
}
...
* @param    Data is the data sent to be written in the flash memory.
...
int SpiFlashWrite(XSpi *SpiPtr, u32 Addr, char Data[],u32 ByteCount, u8 WriteCmd)
{
	...
	/*
	 * Fill in the TEST data that is to be written into the Flash device.
	 */
	for(Index = READ_WRITE_EXTRA_BYTES; Index < ByteCount + READ_WRITE_EXTRA_BYTES; Index++) {
		WriteBuffer[Index] = Data[Index - READ_WRITE_EXTRA_BYTES];
	}
